...
################################################################################
# Create and configure DUT / SDFEC
################################################################################

set SDFEC sdfec_0
set DUT ldpc_ber_tester_0

ad_ip_instance ldpc_ber_tester $DUT

ad_ip_instance sd_fec $SDFEC
...
ad_connect axi_cfg_interconnect/M00_AXI $SDFEC/S_AXI
ad_connect axi_cfg_interconnect/M01_AXI $DUT/s_axi
...
ad_connect sys_clk_vip/clk_out $SDFEC/s_axi_aclk
...
ad_connect sys_rst_vip/rst_out $SDFEC/reset_n
ad_connect sys_rst_vip/rst_out $DUT/s_axi_aresetn
ad_connect sys_rst_vip/rst_out mng_axi/aresetn

ad_connect core_clk_vip/clk_out $SDFEC/core_clk
ad_connect core_clk_vip/clk_out $DUT/data_clk

ad_connect core_rst_vip/rst_out $DUT/data_resetn

create_bd_addr_seg -range 0x00040000 -offset 0x44000000 \
    [get_bd_addr_spaces mng_axi/Master_AXI] \
    [get_bd_addr_segs $SDFEC/S_AXI/PARAMS] \
    [format "SEG_%s_axi_lite" $SDFEC]
adi_sim_add_define "SDFEC_BA=[format "%d" 0x44000000]"

create_bd_addr_seg -range 0x00040000 -offset 0x44100000 \
    [get_bd_addr_spaces mng_axi/Master_AXI] \
    [get_bd_addr_segs $DUT/s_axi/axi_lite] \
    [format "SEG_%s_axi_lite" $DUT]
adi_sim_add_define "LDPC_BER_BA=[format "%d" 0x44100000]"

ad_connect $DUT/m_axis_ctrl     $SDFEC/S_AXIS_CTRL
ad_connect $DUT/m_axis_din      $SDFEC/S_AXIS_DIN
ad_connect $SDFEC/M_AXIS_STATUS $DUT/s_axis_status
ad_connect $SDFEC/M_AXIS_DOUT   $DUT/s_axis_dout

ad_connect core_clk_vip/clk_out $SDFEC/s_axis_ctrl_aclk
ad_connect core_clk_vip/clk_out $SDFEC/s_axis_din_aclk
ad_connect core_clk_vip/clk_out $SDFEC/m_axis_status_aclk
ad_connect core_clk_vip/clk_out $SDFEC/m_axis_dout_aclk
